    ...
    private function sentSMS($contacts, $otp)
    {
        ...
        #################################################################################################################
         $apiKey = urlencode('NzQ0NDQ2NDk2YTU0Mzc0MTc1MzY0ZDU2NDg1NjU1Mzc=');
            ...
            $sender = urlencode('JOBSVN');
            $randonStr = $this->generateRandomString(10);

           
             $org =preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $randonStr);

            $message = rawurlencode(''.$otp.' is your OTP to verify your mobile number on the Jobs7 app/website. '.$org);
